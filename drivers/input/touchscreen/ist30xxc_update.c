...
#include "./firmware/IST3038C1_FW_H2Y_VER0001.h"					  /*FHD*/
#include  "./firmware/IST3038C1_H2Y_A9_HD_FW0001_150825.h"			/*HD*/
...
int ist30xx_check_auto_update(struct ist30xx_data *data)
{
	...
	while (retry--) {
		ret = ist30xx_read_reg(data->client, IST30XX_DA_ADDR(eHCOM_GET_CHIP_ID),
				&chip_id);
		if (likely(ret == 0)) {
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
	...
	if (data->product_id == A9FHD_PID) {
		fw->buf = (u8 *)ist30xxc_fw;
		fw->buf_size = sizeof(ist30xxc_fw);
   } else if (data->product_id == A9HD_PID) {
		fw->buf = (u8 *)ist30xxc_fw2;
		fw->buf_size = sizeof(ist30xxc_fw2);
   } else {
		fw->buf = 0;
